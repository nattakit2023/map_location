<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark">Performance Report
                </h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="<?= base_url(); ?>/dashboard">Dashboard</a></li>
                    <li class="breadcrumb-item active">Performance Report</li>
                </ol>
            </div>
        </div>
    </div>
</div>
<section class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="card rounded-0">
                    <div class="card-header bg-light rounded-0">
                        <h3>
                            Add Performance Report
                        </h3>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-11">
                                <div class="col-md-10">
                                    <strong style="font-size: 18px;">Select file </strong>
                                </div>
                                <div class="col-md-10">
                                    <input type="file" name="files" id="files" accept="image/jpeg, image/png, image/gif, image/tiff, image/svg+xml, application/pdf" multiple>
                                </div>
                            </div>
                            <div class="col-md-1" style="align-content: end;">
                                <button id="addFilesFMS" class="btn btn-block btn-primary rounded-0"><i class="fas fa-plus"></i> Add File</button>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="row">
            <div class="col-md-12">
                <div class="card rounded-0">
                    <div class="card-header bg-light rounded-0">
                        <h3>
                            Performance Report List
                        </h3>
                    </div>
                    <div class="card-body">
                        <table id="fmslisttbl" class="table card-table table-vcenter text-nowrap">
                            <thead>
                                <tr>
                                    <th>File Name</th>
                                    <th>File Type</th>
                                    <th>Uploaded Date</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (!empty($fms)) {
                                    foreach ($fms as $fms_data) {
                                ?>
                                        <tr>
                                            <td><?php echo $fms_data['file_name']; ?></td>
                                            <td><?php echo $fms_data['file_type']; ?></td>
                                            <td><?php echo $fms_data['timestamp']; ?></td>
                                            <td>
                                                <a style="color: green;" href="<?php echo base_url($fms_data['file_path']); ?>" target="_blank"><i class="fas fa-eye"></i></a>
                                                <?php if ($fms_data['file_type'] == 'pdf') { ?>
                                                    <a href="<?php echo base_url($fms_data['pdf_path']); ?>" target="_blank"><i class="fas fa-file"></i></a>
                                                <?php } ?>
                                                <a style="color: red;" href="#" onclick="deletePerformance(<?= $fms_data['id'] ?>)"><i class="fas fa-trash"></i></a>
                                            </td>
                                        </tr>
                                        </tr>
                                <?php }
                                } ?>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>

</section>

<script>
    $(document).on('click', '#addFilesFMS', function() {
        var formdata = new FormData();

        let files = $('#files')[0].files;

        if (files.length == 0) {
            alert('Please Input or Select All Fields');
            return false;
        }

        for (var index = 0; index < files.length; index++) {
            formdata.append('files[]', files[index]);
        }

        $.ajax({
            url: '<?= base_url(); ?>reports/add_performance',
            type: 'POST',
            data: formdata,
            contentType: false,
            processData: false,
            success: function(response) {
                location.reload();
            }
        });
    })

    function deletePerformance(performance_id) {
        if (confirm('Are you sure you want to delete this Performance report?')) {
            $.ajax({
                url: '<?= base_url(); ?>reports/delete_performance',
                type: 'POST',
                data: {
                    id: performance_id
                },
                success: function(response) {
                    location.reload();
                }
            });
        }
    }
</script>