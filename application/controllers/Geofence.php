...
class Geofence extends CI_Controller
{
	...
	{
		$response = $this->geofence_model->add_geofence($this->input->post('data'));
		if($response) {
	...
	public function geofence_management(){
		$geodata = array();
		$geofencelist = $this->geofence_model->getall_geofence();
		if (!empty($geofencelist)) {
			foreach ($geofencelist as $key => $geofencelists) {
		...
		$data['geofencelist'] = $geodata;
		$this->template->template_render('geofence_management', $data);
	}
